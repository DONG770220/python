"""
整合版：鸢尾花数据集分类可视化综合分析
包含数据预览、分类器对比、3D决策边界、3D概率图和特征重要性分析
所有模型训练逻辑保持不变，仅优化可视化呈现
"""
import numpy as np
import pandas as pd
...
df_clean = df.dropna()  # 处理缺失值

# 为分类任务准备数据（使用后两个特征用于2D可视化）
X_2d = iris.data[:, 2:]  # 花瓣长度和宽度
...
# ----------------------------
print("===== 数据预览 =====")
print(df_clean.sample(5))  # 随机展示5条数据
...
plt.tight_layout(rect=[0, 0.05, 1, 1])  # 预留图例空间
